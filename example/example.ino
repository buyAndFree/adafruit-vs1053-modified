...
Adafruit_VS1053_FilePlayer musicPlayer = Adafruit_VS1053_FilePlayer(SHIELD_RESET, SHIELD_CS, SHIELD_DCS, DREQ, CARDCS); // don't alter
...
void setup() {
  ...
  musicPlayer.applyPatch(plugin, PLUGIN_SIZE);
  if (!SD.begin(CARDCS)) {
...
void loop() {
  if (! musicPlayer.startPlayingFile("001.mp3")) {
    Serial.print("Failed to open mp3 file: 001.mp3");
    while (1);
  }
  Serial.println("Started playing");
  while (musicPlayer.playingMusic) {
    int numOfBands = 14; // change this to the number of bands being used, default is 14, vs1053 supports up to 23
    uint16_t currentVal[numOfBands] = { 0 };
    uint16_t peak[numOfBands] = { 0 };
    musicPlayer.readFrequencies(currentVal, peak);
    //do stuff with the values which are now in currentVal and peak
    //e.g. light up leds, or trigger something if a threshold passed on a certain frequency band

    //following code will just print out the values
    Serial.print("Current Values: ");
    for (int i=0; i<14; i++) {
      Serial.print(currentVal[i]);
      Serial.print(",");
    }
    Serial.println();
    ...
    delay(50); // suitable timedelay between each 'readFrequencies' is from 5 to 20 times per second (i.e. delay of 50ms to 200ms)
  }
}
