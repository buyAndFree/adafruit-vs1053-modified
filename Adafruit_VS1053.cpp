...
void Adafruit_VS1053::readFrequencies(uint16_t *currentVal, uint16_t *peak) {
	sciWrite(VS1053_REG_WRAMADDR, VS1053_BASE+2);
	uint16_t bands = sciRead(VS1053_REG_WRAM); 
	sciWrite(VS1053_REG_WRAMADDR, VS1053_BASE+4);
	for (uint16_t i=0;i<bands;i++) {
		uint16_t buf = sciRead(VS1053_REG_WRAM); 
		/* current value in bits 5..0, normally 0..31
		   peak value in bits 11..6, normally 0..31 */
		currentVal[i] = buf&0x003F;
		peak[i] = (buf>>6)&0x003F;
	}
}
void Adafruit_VS1053::checkRateAndBands(void) {
	sciWrite(VS1053_REG_WRAMADDR, VS1053_BASE+1);
	uint16_t rate = sciRead(VS1053_REG_WRAM);
	uint16_t bands = sciRead(VS1053_REG_WRAM);
	Serial.print("Sampling Rate: "); Serial.println(rate);
	Serial.print("Number Of Bands: "); Serial.println(bands);
}

boolean Adafruit_VS1053::setFrequencies(int numOfBands, short *frequency) {
	sciWrite(VS1053_REG_WRAMADDR, VS1053_BASE+0x68);
	if (numOfBands < 1 || numOfBands > 23) {
		return false;
	}
	for (int i = 0; i < numOfBands; i++) {
		sciWrite(VS1053_REG_WRAM, frequency[i]);
	}
	if (numOfBands < 23) {
		sciWrite(VS1053_REG_WRAM, 25000);
	}
	sciWrite(VS1053_REG_WRAMADDR, VS1053_BASE+1);
	sciWrite(VS1053_REG_WRAM, 0);
	return true;
...
uint16_t Adafruit_VS1053::readBands(void) {
	sciWrite(VS1053_REG_WRAMADDR, VS1053_BASE+2);
	uint16_t bands = sciRead(VS1053_REG_WRAM);
	return bands;
}

uint16_t Adafruit_VS1053::readSamplingRate(void) {
	sciWrite(VS1053_REG_WRAMADDR, VS1053_BASE+1);
	uint16_t rate = sciRead(VS1053_REG_WRAM);
	return rate;
}
